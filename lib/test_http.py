...
class TestHttp(unittest.TestCase):
    ...
    def test_get_with_https(self):
        with http.get("https://badgeserver.emfcamp.org") as response:
            self.assertEqual(response.status, 200)

    def test_get(self):
        with http.get("http://httpbin.org/get", params={"foo": "bar"}, headers={"accept": "application/json"}) as response:
    ...
    def test_post_form(self):
        with http.post("http://httpbin.org/post", data={"foo": "bar"}).raise_for_status() as response:
            content = response.json()
    ...
    def test_post_string(self):
        with http.post("http://httpbin.org/post", data="foobar").raise_for_status() as response:
            content = response.json()
            self.assertEqual(content["headers"]["Content-Type"], "text/plain; charset=UTF-8")
            self.assertEqual(content["data"], "foobar")

    def test_post_json(self):
        with http.post("http://httpbin.org/post", json={"foo":"bar"}).raise_for_status() as response:
            content = response.json()
